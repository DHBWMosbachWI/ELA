...
if __name__ == "__main__":
    ...
    parser.add("--predefined_elements", nargs='+', required=True)
    ...
    if gen_train_data:
        ...
        unlabeled_data_df.to_csv(join(
            ...
            "check_elements_in_col", "out", "results",
            f"{corpus}_check_elements_in_col_results_{sem_type}_{PERCENTAGE_OF_ELEMENTS_IN_COL}_{labeled_data_size}_{unlabeled_data_size}_{test_data_size}_{random_state}.csv"
        ),
                                 index=False)
        ...
        class_reportable_data[[
            ...
            os.environ["WORKING_DIR"], "labeling_functions",
            "check_elements_in_col", "out", "gen_training_data",
        ...
        with open(
                join(
                    os.environ["WORKING_DIR"], "labeling_functions",
                    "check_elements_in_col", "out", "validation",
    ...
    ########################################
    ## Validation on / no gen training data
    ########################################
    else: # if not gen_train_data
        
        # filter out test data from valid_headers
        ...
        L_train = parallelize(test_data_df, applier.apply, n_worker)
        ...
        test_data_df.to_csv(join(
            os.environ["WORKING_DIR"], "labeling_functions",
            "check_elements_in_col", "out", "results",
            f"{corpus}_check_elements_in_col_results_test_{sem_type}_{PERCENTAGE_OF_ELEMENTS_IN_COL}_{labeled_data_size}_{unlabeled_data_size}_{test_data_size}_{random_state}.csv"
        ),
                                 index=False)
