...
word_vectors_f = open(os.path.join(
    embedding_loc, 'glove.6B.50d.txt'), encoding="utf-8")
word_to_embedding = {}
...
def extract_word_embeddings_features(values):
    ...
    for v in values:
        ...
        if v in word_to_embedding:
            ...
            if embeddings_to_all_words:
                mean_of_word_embeddings = np.nanmean(
                    embeddings_to_all_words, axis=0)
    ...
    if len(embeddings) == 0:
        ...
        mode_embeddings = stats.mode(
            embeddings, axis=0, nan_policy='omit')[0].flatten()
