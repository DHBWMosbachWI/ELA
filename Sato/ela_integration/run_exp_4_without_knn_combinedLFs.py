    ...
    for index, percent in enumerate([1,2,3,4,5]):
        for distance_threshold in [1e-2]:
            for random_state in [1,2,3,4,5]:
                comment = f"embclus_all_combined_{snorkel_label_model}_labeled{percent}_unlabeledAbsolute_test{test_data_size}_{random_state}"
                ...
                #embclus_gen_train_data_path = f"D:\\semantic_data_lake\\semantic_data_lake\\emb_clus\\without_knn\\out\\gen_training_data\\{corpus}_gen_training_data_{distance_threshold}_{percent}_absolute_{test_data_size}_{random_state}.csv"
                embclus_gen_train_data_path = f"D:\\ELA\\labeling_functions\\combined_LFs\\gen_training_data\\{corpus}_gen_training_data_all_combined_{snorkel_label_model}_{percent}_absolute_{test_data_size}_{random_state}.csv"
                pretrained_shelock_path = f"sherlock_retrain_embclus_all_combined_{snorkel_label_model}_labeled{percent}_unlabeledAbsolute_test{test_data_size}_{random_state}.pt"
                
                if os.environ["TYPENAME"] == "type78":
                    pretrained_CRF_LDA_path = f"CRF+LDA_retrain_embclus_all_combined_{snorkel_label_model}_labeled{percent}_unlabeledAbsolute_test{test_data_size}_{random_state}.pt"
                else:
                    pretrained_CRF_LDA_path = f"CRF+LDA_retrain_embclus_all_combined_{snorkel_label_model}_labeled{percent}_unlabeledAbsolute_test{test_data_size}_{random_state}_train100.pt"
                # retrain sherlock
                os.system(
